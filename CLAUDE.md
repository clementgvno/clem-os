...
- **AI** : `@convex-dev/agent` backend (default model `z-ai/glm-5.3-flash`, override via `OPENROUTER_MODEL`) + `@assistant-ui/react` front + streaming HTTP route `/api/chat`. Provider abstracted via `getModel()` in `convex/agent.ts`. The chat agent ships with **DB-acting tools** (`convex/agentTools.ts`) scoped to the thread's org: list/create/update/delete `items`.
